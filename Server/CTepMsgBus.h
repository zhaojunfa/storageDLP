#pragma once
#include "..\include\msgbus\IMsgBus.h"
#include "..\include\msgbus\IMsgReceiver.h"
#include "..\..\TEPCode\interface\kxml\kstr.h"
	...
	VOID LoadAllPlugin()
	{
		...
		for(int i = 0; key.EnumKey(i, subKeyName, &dwLength) != ERROR_NO_MORE_ITEMS; dwLength = MAX_PATH, i++)
		{
			...
			if( ERROR_SUCCESS == subKey.Open(HKEY_LOCAL_MACHINE, subKeyPath))
			{
				...
				OutputDebugString(L"TepServer bus: LoadName：" + info.name);
				len = MAX_PATH;
				ZeroMemory(value, MAX_PATH);
					...
					if(info._hModle != INVALID_HANDLE_VALUE)
					{
						...
						if (p)
						{
							...
							if (info._plugin != NULL)
							{
								...
								OutputDebugString(L"TepServer bus: _plugin->GetFilter " + info.filter);
								
								OutputDebugString(L"TepServer bus: info.name：" + info.name);
								PluginMap.SetAt(info.name, info);
								
							}
							else
								OutputDebugString(L"TepServer bus: 失败！！！加载插件：" + info.name);
