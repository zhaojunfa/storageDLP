...
void CTrayHostDlg::OnInit()
{
	...
	if(0)
	{
		for(int i=0; i<10; i++)
		{
			...
			pGridCtrl->SetSubItem(nRow, 2, _T("敏感文件"));
...
void CTrayHostDlg::InitControls()
{
	...
	if (pGrid)
	{
		...
		if (connectState == L"0")
		{
			connectState = L"服务器连接正常";
		}
		else
		{
			connectState = L"服务器连接异常";
		}
...
LRESULT CTrayHostDlg::OnDuiMsgGridCtrlDropFile(UINT uID, CString strName, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	if(pGridCtrl)
	{
		...
		int nRow = pGridCtrl->InsertRow(-1,	// 插入的行序号,-1表示添加到最后
			...
			-1,								// 行左侧图片(索引图片方式,无索引图片填-1)
			Color(0, 0, 0, 0),				// 行文字颜色,全0表示默认(不使用行文字颜色,使用表格全局颜色)
			_T("Skins\\skins\\icon\\NewIcons005.png"),	// 行左侧的图片文件
			-1,								// 行右侧图片(索引图片方式,无索引图片填-1)
			_T(""),							// 行右侧的图片文件
			0);
...
// 						}
					...
					{
						MessageBox(NULL, L"恢复失败！\n路径：" + item.IsolatePath, L"错误", MB_ICONERROR | MB_OK);
					}
				}
			}
		}
...
LRESULT CTrayHostDlg::OnDlgSetServerMouseDown( UINT uID, CString strName, UINT Msg, WPARAM wParam, LPARAM lParam )
{
	...
	if (!ip.IsEmpty())
	{
		...
		{
			DuiSystem::DuiMessageBox(NULL, _T("\n设置成功！"), _T("服务器设置"), MB_OK, 200, 150);
			base->DoOK();
		}
		else
			DuiSystem::DuiMessageBox(NULL, _T("\n设置失败！"), _T("服务器设置"), MB_ICONWARNING, 200, 150);
	}
...
LRESULT CTrayHostDlg::OnDlgSetScanMouseDown( UINT uID, CString strName, UINT Msg, WPARAM wParam, LPARAM lParam )
{
	...
	if (!dir.IsEmpty())
	{
		...
		if (msg.GetValueI32(L"./result") == 1)
		{
			DuiSystem::DuiMessageBox(NULL, _T("\n设置成功！"), _T("扫描路径设置"), MB_OK, 200, 150);
			base->DoOK();
		}
		else
			DuiSystem::DuiMessageBox(NULL, _T("\n设置失败！"), _T("扫描路径设置"), MB_ICONWARNING, 200, 150);
	}
	InitControls();
	return S_OK;
}
...
LRESULT CTrayHostDlg::OnDlgSetSafeMouseDown( UINT uID, CString strName, UINT Msg, WPARAM wParam, LPARAM lParam )
{
	...
	if (!dir.IsEmpty())
	{
		...
		if (msg.GetValueI32(L"./result") == 1)
		{
			DuiSystem::DuiMessageBox(NULL, _T("\n设置成功！"), _T("安全目录设置"), MB_OK, 200, 150);
			base->DoOK();
		}
		else
			DuiSystem::DuiMessageBox(NULL, _T("\n设置失败！"), _T("安全目录设置"), MB_ICONWARNING, 200, 150);
	}
	InitControls();
	return S_OK;
}
...
LRESULT CTrayHostDlg::OnDlgMaxThreadMouseDown( UINT uID, CString strName, UINT Msg, WPARAM wParam, LPARAM lParam )
{
	...
	if (!count.IsEmpty())
	{
		...
		if (msg.GetValueI32(L"./result") == 1)
		{
			DuiSystem::DuiMessageBox(NULL, _T("\n设置成功，重启后生效！"), _T("最大并发设置"), MB_OK, 200, 150);
			base->DoOK();
		}
		else
			DuiSystem::DuiMessageBox(NULL, _T("\n设置失败！"), _T("最大并发设置"), MB_ICONWARNING, 200, 150);
	}
...
LRESULT CTrayHostDlg::OnDlgSetIsoMouseDown( UINT uID, CString strName, UINT Msg, WPARAM wParam, LPARAM lParam )
{
	...
	if (!dir.IsEmpty())
	{
		...
		if (msg.GetValueI32(L"./result") == 1)
		{
			DuiSystem::DuiMessageBox(NULL, _T("\n设置成功！"), _T("隔离目录设置"), MB_OK, 200, 150);
			base->DoOK();
		}
		else
			DuiSystem::DuiMessageBox(NULL, _T("\n设置失败！"), _T("隔离目录设置"), MB_ICONWARNING, 200, 150);
	}
